...
for element in studentsArr{
    let studentGroupArr = element.components(separatedBy: " - ")
    students.append(studentGroupArr[0])
    groups.append(studentGroupArr[1])
}
...
for i in 0...students.count - 1{
    studentsGroups.append(students[i], key: groups[i])
}
for k in studentsGroups.keys{
    studentsGroups[k]?.sort()
}

//studentsGroups.sorted(by: { $1.0 < $0.0 })
...
for group in studentsGroups.keys {
    ...
    for student in studentsGroups[group]! {
        ...
        for point in points {
            studentPoints[group]?[student]?.append(randomValue(maxValue: point))
        }
    }
}
// Your code ends
...
for group in studentPoints.keys {
    ...
    for student in studentPoints[group]!.keys {
        var sumPoint = 0
        for point in studentPoints[group]![student]! {
            sumPoint += point
        }
...
for group in sumPoints.keys {
    var sumPointsInGroup = 0
    for point in sumPoints[group]!.values {
        sumPointsInGroup += point
    }
    groupAvg[group] = Float(sumPointsInGroup)/Float(sumPoints[group]!.values.count)
}
...
class TimeAB{
    ...
    func getSum (time : TimeAB) -> TimeAB? {
        ...
        return try! TimeAB(hours: self.hours, minutes: self.minutes, seconds: self.seconds)
    }
    func getDif(time : TimeAB) -> TimeAB? {
        self.hours -= time.hours
        self.minutes -= time.minutes
        self.seconds -= time.seconds
    ...
    func getSumOfTwoTimes (time1 : TimeAB, time2 : TimeAB) -> TimeAB? {
        ...
        return try! TimeAB(hours: hoursSum, minutes: minutesSum, seconds: secondsSum)
    }
    func getDifOfTwoTimes(time1 : TimeAB, time2 : TimeAB) -> TimeAB? {
        ...
        return try! TimeAB(hours: hoursDif, minutes: minutesDif, seconds: secondsDif)
    }
}

var time0 = TimeAB()
var time1 = try! TimeAB(hours: 3, minutes: 30, seconds: 45)
var time2 = try! TimeAB(hours: 15, minutes: 45, seconds: 55)

let date = Date()
var timeFromDate = TimeAB(date: date)

//print times in AM|PM format
print("\n-----PART2-----\n")
...
print("Time1 + Time2 = " + sum!.getTimeInfoOrdinary() + "\n")   //print time in 24-hours format

var sumOfTwoTimes = TimeAB().getSumOfTwoTimes(time1: time2, time2: timeFromDate)
...
var difOfTwoTimes = TimeAB().getDifOfTwoTimes(time1: time1, time2: time0)
